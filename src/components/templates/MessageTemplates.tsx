...
const MessageTemplates = ({ wabaId }: MessageTemplatesProps) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (selectedId === deleteTemplateId) {
        setSelectedId(templates.find((t) => t._id !== deleteTemplateId)?._id || null);
      }
  ...
  return (
    <div className="sm:p-8 p-5 space-y-8 h-[calc(100vh-2rem)] flex flex-col">
      <CommonHeader backBtn title="Manage Templates" description="Manage and design your WhatsApp message templates" featureKey="template_bots_used" onRefresh={handleRefresh} onSync={handleSync} onSyncStatus={handleSyncStatus} isLoading={isLoading} isSyncingStatus={isSyncingStatus} onAddClick={() => router.push(`/templates/${wabaId}/create`)} addLabel="Create New Template" onSearch={setSearchQuery} searchTerm={searchQuery} searchPlaceholder="Search templates..." rightContent={WabaDisplay}>
